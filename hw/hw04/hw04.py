...
def balanced(m):
    ...
    if is_planet(m):
        return True
    moment1 = total_weight(end(left(m))) * length(left(m))
    moment2 = total_weight(end(right(m))) * length(right(m))
    return balanced(end(left(m))) and balanced(end(right(m))) and moment1 == moment2
...
def quadratic(x, a, b, c):
    ...
    if x1 <= delta <= x2:
        if a > 0:
            return interval(f(delta), max(f1, f2))
        else:
            return interval(min(f1, f2), f(delta))
    else:
...
def check_par():
    ...
    True
    """
    r1 = interval(1, 100000000) # Replace this line!
    r2 = interval(1, 100000000) # Replace this line!
    return r1, r2
