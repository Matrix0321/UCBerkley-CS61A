...
def g_iter(n):
    ...
    if n <= 3:
        ...
        g1,g2,g3,i = 1,2,3,3
        while i < n:
            g1,g2,g3,i = g2, g3, 1*g3 + 2*g2 + 3*g1, i + 1
        return g3
...
def count_change(total):
    ...
    """
    """
    Another method which is true but not allowed in this question. 
    Functions should not contain 'for' or 'while'.
    arr = []
    i = 1
    while i <= total:
        arr.append(i)
    ...
    for i in range(len(arr)):
        for j in range(arr[i], total + 1):
            res[j] = res[j] + res[j-arr[i]] # sum arr[i] and including a[[i]]
    return res[total]
    """
    def helper(n, m):
        if n == 0:
...
def move_stack(n, start, end):
    ...
    "*** YOUR CODE HERE ***"
    #Can't solve it currently. Maybe later.
